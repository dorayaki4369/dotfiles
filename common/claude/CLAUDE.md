...
コード生成やセットアップ、設定などをする場合は、**常にContext7を使用して**ライブラリやフレームワークの最新情報を参照すること。
作業を開始する前に、必ずライブラリの最新情報を確認すること。古い情報に基づいた実装は**絶対に**避けなければならない。

### 不明点は確認する

要件や設計意図など、コードベースから判断できない事項は、推測で進めずにユーザーに確認して解決する。

### 実装はテストする

ユニットテストやインテグレーションテストは必ず書き、実装が正しいかを継続的に検証する。

Web画面のUIをテストする際は、**chrome-devtools**を使用して実際に表示されるUIを確認すること。

### コミットはトピックごとに分ける

複数のトピックの編集内容がある場合は、コミットを分割すること。
...
ライブラリで既に定義されている型を再定義するのを止めること。（車輪の再発明の禁止）
型情報が不明な場合は、積極的に調査すること。
 ...
 * データ検索
 *
 * フィルター条件を元に、データを検索します。
 *
 * @param filter - フィルター条件
 * @returns データ検索結果のリスト
 */
export async function search(filter?: Schemas['filter']): Promise<Data[]> {
  // 検索処理
}
```
...
何を処理することを目的にした関数なのかをコメントに記載し、実装を見なくても関数がどのような処理を行なっているのかわかるようにする。
...
ユーザーとClaude Codeが会話するときに守るルールについて。

### ダイアグラム表示はASCII artやテキストで直接図を描画する

コンソール（Claude Codeの出力）でダイアグラムを説明する場合は、mermaidコードブロックではなく、**ASCII artやテキストで直接図を描画**すること。コンソールでは図がレンダリングされないため、視覚的に理解しやすいテキスト図を使用する。
...
Markdownファイルに書き込む場合は、従来通りmermaidコードブロックを使用する。
